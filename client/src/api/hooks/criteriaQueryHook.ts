...
export const useFetchCriteria = () => {
  return useQuery<Criterion[]>(['fetchCriteria'], async () => (await axios.get(`${API_HOST}/GetCriteria`)).data, { retry: false })
}
...
export const useFetchCriterion = (criterionId: number, onSuccess: (data: CriterionDetails) => void) => {
  return useQuery<CriterionDetails>(
    ...
    async () => (await axios.get(`${API_HOST}/GetCriterion/${criterionId}`)).data,
